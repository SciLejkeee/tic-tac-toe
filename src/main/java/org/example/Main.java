...
public class Main {
    ...
    private static String GAME_STATE_0_WON = "0 победили";
    ...
    public static void main(String[] args) {
        System.out.println("Приветствую, игрок!");
        while (true) {
            startGameRound();
        }
    }
    ...
    public static int[] inputCellCoordinates(String[][] board) {
        System.out.println("Введите 2 числа от 0 до 2 через пробел:");

        do {
            //допущения не проверяем на наличие пробела и корректносмть цифр
    ...
    public static String checkGameState(String[][] board) {
        ...
        for (int col = 0; col < COL_COUNT; col++) {
            int colSum = 0;
            for (int row = 0; row < ROW_COUNT; row++) {
                colSum += calculateNumValue(board[row][row]);
            }
            sums.add(colSum);
        }
